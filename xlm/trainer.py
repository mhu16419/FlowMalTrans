...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.epoch_size = params.epoch_size * params.accumulate_gradients # if accumulate gradients is more than 1, maybe we want more setences in one epoch
        ...
        if params.multi_gpu and params.amp == -1:
            ...
            for name in self.MODEL_NAMES:
                setattr(self, name, nn.parallel.DistributedDataParallel(getattr(self, name), device_ids=[params.local_rank], output_device=params.local_rank, broadcast_buffers=False, find_unused_parameters=True))
        ...
        self.reload_checkpoint(params.is_checkpoint)
    ...
    def reload_checkpoint(self, checkpoint=True):
        """
        ...
        # if load checkpoint where optimizers are available
        if checkpoint:
            # reload optimizers
            for name in self.optimizers.keys():
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        if params.separate_decoders:
            if lang2 == lang1:
                model = self.src_decoder
            else:
                model = self.tgt_decoder
        else:
            model = self.decoder
        model.train()
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params, src_flow=None, tgt_flow=None, shared_h2z_projection=None):
        ...
        if not params.separate_decoders:
            ...
            self.src_decoder = decoder[0]
            self.tgt_decoder = decoder[1]
            self.MODEL_NAMES = ['encoder', 'src_decoder', 'tgt_decoder']

        self.src_flow = src_flow  # for lang1
        self.tgt_flow = tgt_flow  # for lang2
        self.shared_h2z_projection = shared_h2z_projection
        self.data = data
        self.params = params

        # considering the used languages for the flows
        used_langs = params.lgs.split('-')
        assert len(used_langs) == 2
        self.used_lang1 = used_langs[0]  # for src_flow and optionally for src_decoder
        self.used_lang2 = used_langs[1]  # for tgt_flow and optionally for tgt_decoder

        if params.use_latent_code and params.use_h2z_matrix:
            self.MODEL_NAMES.append('shared_h2z_projection')

        if params.use_flow:
            self.MODEL_NAMES.append('src_flow')
            self.MODEL_NAMES.append('tgt_flow')
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        self.encoder.train()
        if params.separate_decoders:
            self.src_decoder.train()
            self.tgt_decoder.train()
        else:
            self.decoder.train()

        if params.use_latent_code and params.use_h2z_matrix:
            self.shared_h2z_projection.train()
        ...
        # print(x1.size())  # LongTensor(slen, bs)
        # print(enc1.size())  #(bs, slen, emb_dim)
        # enc1 should now have the size (batch_size, seq_len, embedding_dim)

        # obtain the sentence-level latent code from enc1
        z = None
        if params.use_latent_code:
            use_mean_pooling = params.use_mean_pooling
            use_max_pooling = params.use_max_pooling
            use_CSL = params.use_CSL
            if not use_mean_pooling and not use_max_pooling and not use_CSL:
                raise ValueError('There must at least be one method to obtain the sentence level latent code')
            if params.use_mean_pooling:
                # h_enc: [B, Embedding dim]
                h_enc_mean = F.avg_pool1d(enc1.transpose(1, 2).contiguous(),
                                          enc1.size(1)).squeeze(2)
            if params.use_max_pooling:
                h_enc_max = F.max_pool1d(enc1.transpose(1, 2).contiguous(),
                                         enc1.size(1)).squeeze(2)
            if params.use_CSL:
                h_enc_CSL = enc1[:, 0]

            # whether concat the hs or add the hs
            if params.concat_h:
                if params.use_mean_pooling:
                    h_enc = h_enc_mean
                    if use_max_pooling:
                        h_enc = torch.cat([h_enc, h_enc_max], dim=-1)
                    if use_CSL:
                        h_enc = torch.cat([h_enc, h_enc_CSL], dim=-1)
                elif params.use_max_pooling:
                    h_enc = h_enc_max
                    if use_CSL:
                        h_enc = torch.cat([h_enc, h_enc_CSL], dim=-1)
                else:  # only use the CSL
                    h_enc = h_enc_CSL
            else:
                if params.use_mean_pooling:
                    h_enc = h_enc_mean
            ...
            # check whether projection from n*emb_dim to z_size is needed
            if params.use_h2z_matrix:
                z = self.shared_h2z_projection(h_enc)
            else:
                z = h_enc

            if params.use_flow:
                if lang1 == self.used_lang1:
                    z_k, log_det = self.src_flow(z.detach().clone(), direction='fwd')
                    eps = z_k[-1]
                elif lang1 == self.used_lang2:
                    z_k, log_det = self.tgt_flow(z.detach().clone(), direction='fwd')
                    eps = z_k[-1]
                else:
                    raise ValueError('The language has to be src or tgt.')

                log_prob_prior = -1 / 2 * (math.log(2 * math.pi) + eps.pow(2)).sum(-1)  # [B]
                log_density = log_prob_prior + log_det
                nll_loss = -log_density.mean() * params.nll_weight
                # self.optimize(nll_loss)

        # print(z.size())  # [bs, z_size]
        # decode target sentence
        if params.separate_decoders:
            if lang2 == self.used_lang1:
                dec2 = self.src_decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                _, loss = self.src_decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False, z=z)
            elif lang2 == self.used_lang2:
                dec2 = self.tgt_decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                _, loss = self.tgt_decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False, z=z)
            else:
                raise ValueError('The language does not correspond to any decoders.')
        else: 
            dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
            _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False, z=z)

        # dec2: [tgt_seq_len, batch_size, emb_dim]
        # y: [(tgt_seq_len-1)*batch_size]
        # loss
        self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss

        # optimize
        if params.use_flow and params.use_latent_code:
            self.optimize(nll_loss + loss)
        else:
            self.optimize(loss)
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        if params.separate_decoders:
            _src_decoder = self.src_decoder.module if params.multi_gpu else self.src_decoder
            _tgt_decoder = self.tgt_decoder.module if params.multi_gpu else self.tgt_decoder
        else:
            _decoder = self.decoder.module if params.multi_gpu else self.decoder

        if params.use_latent_code and params.use_h2z_matrix:
            _shared_h2z_projection = self.shared_h2z_projection.module if params.multi_gpu else self.shared_h2z_projection

        if params.use_flow:
            _src_flow = self.src_flow.module if params.multi_gpu else self.src_flow
            _tgt_flow = self.tgt_flow.module if params.multi_gpu else self.tgt_flow
        ...
        with torch.no_grad():
            ...
            self.encoder.eval()
            if params.separate_decoders:
                self.src_decoder.eval()
                self.tgt_decoder.eval()
            else:
                self.decoder.eval()

            if params.use_latent_code and params.use_h2z_matrix:
                self.shared_h2z_projection.eval()
            ...
            if params.use_latent_code:
                ...
                # check whether projection from n*emb_dim to z_size is needed
                if params.use_h2z_matrix:
                    z = _shared_h2z_projection(h_enc)
                else:
                    z = h_enc

                # here is from lang1 to lang2
                if params.use_flow:
                    if lang1 == self.used_lang1 and lang2 == self.used_lang2:
                        z_k, _ = _src_flow(z)
                        eps = z_k[-1]
                        z_k_tgt, _ = _tgt_flow(eps, direction='gen')
                        z = z_k_tgt[-1]
                    elif lang1 == self.used_lang2 and lang2 == self.used_lang1:
                        z_k, _ = _tgt_flow(z)
                        eps = z_k[-1]
                        z_k_tgt, _ = _src_flow(eps, direction='gen')
                        z = z_k_tgt[-1]

            if params.separate_decoders:
                if lang2 == self.used_lang1:
                    x2, len2 = _src_decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), z=z)
                elif lang2 == self.used_lang2:
                    x2, len2 = _tgt_decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), z=z)
                else:
                    raise ValueError('The language does not correspond to any decoders.')
            else:
                x2, len2 = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), z=z)
            langs2 = x2.clone().fill_(lang2_id)
        ...
        if params.use_latent_code:
            ...
            if params.use_mean_pooling:
                # h_enc: [B, Embedding dim]
                h_enc_mean = F.avg_pool1d(enc2.transpose(1, 2).contiguous(),
                                          enc2.size(1)).squeeze(2)
            if params.use_max_pooling:
                h_enc_max = F.max_pool1d(enc2.transpose(1, 2).contiguous(),
                                         enc2.size(1)).squeeze(2)
            if params.use_CSL:
                h_enc_CSL = enc2[:, 0]
            ...
            # here is from lang2 to lang3
            # lang3 == lang1
            if params.use_flow:
                z_temp = z.detach().clone()
                if lang2 == self.used_lang1 and lang3 == self.used_lang2:
                    # .detach().clone()
                    if params.detach_type == 'none':
                        z_k, log_det = self.src_flow(z)
                    else:
                        z_k, log_det = self.src_flow(z.detach().clone())
                    eps = z_k[-1]
                    z_k_tgt, _ = self.tgt_flow(eps, direction='gen')
                elif lang2 == self.used_lang2 and lang3 == self.used_lang1:
                    if params.detach_type == 'none':
                        z_k, log_det = self.tgt_flow(z)
                    else:
                        z_k, log_det = self.tgt_flow(z.detach().clone())
                    eps = z_k[-1]
                    z_k_tgt, _ = self.src_flow(eps, direction='gen')
                else:
                    raise ValueError('The language does not correspond to any flow')
                if params.detach_type == 'all':
                    z = z_k_tgt[-1].detach().clone()
                else:
                    z = z_k_tgt[-1]

                # if also consider nll loss in the back translation
                if params.consider_nll_in_bt:
                    if lang2 == self.used_lang1:
                        z_k, log_det = self.src_flow(z_temp)
                    elif lang2 == self.used_lang2:
                        z_k, log_det = self.tgt_flow(z_temp)
                    else:
                        raise ValueError('The language does not correspond to any flow')
                    eps = z_k[-1]
                    log_prob_prior = -1 / 2 * (math.log(2 * math.pi) + eps.pow(2)).sum(-1)  # [B]
        ...
        if params.separate_decoders:
            if lang3 == self.used_lang1:
                dec3 = self.src_decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2)
                _, loss = self.src_decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=False, z=z)
            elif lang3 == self.used_lang2:
                dec3 = self.tgt_decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2)
                _, loss = self.tgt_decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=False, z=z)
            else:
                raise ValueError('The language does not correspond to any decoders.')
        else:
            dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2)
            _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=False, z=z)
        ...
        # optimize
        if params.use_flow and params.consider_nll_in_bt:
            self.optimize(loss + nll_loss)
        else:
            self.optimize(loss)
