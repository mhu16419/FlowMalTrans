...
class EncDecEvaluator(Evaluator):
    ...
    def __init__(self, trainer, data, params):
        ...
        if params.separate_decoders:
            self.src_decoder = trainer.src_decoder
            self.tgt_decoder = trainer.tgt_decoder
        else:
            self.decoder = trainer.decoder
        self.shared_h2z_projection = trainer.shared_h2z_projection

        self.src_flow = trainer.src_flow
        self.tgt_flow = trainer.tgt_flow

        self.used_lang1 = trainer.used_lang1
        self.used_lang2 = trainer.used_lang2
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        if params.separate_decoders:
            src_decoder = self.src_decoder.module if params.multi_gpu else self.src_decoder
            tgt_decoder = self.tgt_decoder.module if params.multi_gpu else self.tgt_decoder
        else:
        ...
        if params.use_latent_code and params.use_h2z_matrix:
            shared_h2z_projection = self.shared_h2z_projection.module if params.multi_gpu else self.shared_h2z_projection

        if params.use_flow:
            src_flow = self.src_flow.module if params.multi_gpu else self.src_flow
            tgt_flow = self.tgt_flow.module if params.multi_gpu else self.tgt_flow
        ...
        # store the log_likelihood_loss for the flows
        nll_loss = 0
        n_val_sentences = 0
            ...
            if params.use_latent_code:
               ...
               # whether concat the hs or add the hs
                ...
                if params.use_h2z_matrix:
                    z = shared_h2z_projection(h_enc)
                else:
                    z = h_enc

                if params.use_flow:
                    if lang1 == self.used_lang1 and lang2 == self.used_lang2:
                        z_k, log_det = src_flow(z)
                        eps = z_k[-1]
                        z_k_tgt, _ = tgt_flow(eps, direction='gen')
                        z = z_k_tgt[-1]
                    elif lang1 == self.used_lang2 and lang2 == self.used_lang1:
                        z_k, log_det = tgt_flow(z)
                        eps = z_k[-1]
                        z_k_tgt, _ = src_flow(eps, direction='gen')
                        z = z_k_tgt[-1]
                    else:
                    ...
                    nll_loss += -log_density.sum()


            # decode target sentence and compute loss
            if params.separate_decoders:
                if lang2 == self.used_lang1:
                    dec2 = src_decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                    word_scores, loss = src_decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True, z=z)
                elif lang2 == self.used_lang2:
                    dec2 = tgt_decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
                    word_scores, loss = tgt_decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True, z=z)
                else:
                ...
                word_scores, loss = decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True, z=z)
                ...
                if params.beam_size == 1:
                    if params.separate_decoders:
                        if lang2 == self.used_lang1:
                            ...
                            decoder = tgt_decoder
                    generated, lengths = decoder.generate(enc1, len1, lang2_id, max_len=max_len, z=z)
                else:
                    if params.separate_decoders:
                        if lang2 == self.used_lang1:
                            decoder = src_decoder
        ...
        # compute the nll of the dataset
        avg_nll_loss = nll_loss / n_val_sentences
        print(avg_nll_loss)
        if params.use_flow:
            scores['%s_%s-%s_nll_loss' % (data_set, lang1, lang2)] = avg_nll_loss.item()
