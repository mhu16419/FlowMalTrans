...
def main(params):
    ...
    #for param_name, param in reloaded.items():
        #logger.info(f"{param_name}")
    # for item in reloaded['dico_word2id'].items():
    #     logger.info(item)
    ...
    for name in ['n_words', 'bos_index', 'eos_index', 'pad_index', 'unk_index', 'mask_index']:
        ...
        '''
        INFO - 06/10/24 11:59:08 - 0:00:01 - n_words:671
        INFO - 06/10/24 11:59:08 - 0:00:01 - bos_index:0
        INFO - 06/10/24 11:59:08 - 0:00:01 - eos_index:1
        INFO - 06/10/24 11:59:08 - 0:00:01 - pad_index:2
        INFO - 06/10/24 11:59:08 - 0:00:01 - unk_index:3
        INFO - 06/10/24 11:59:08 - 0:00:01 - mask_index:5
        '''

    # build dictionary / build encoder / build decoder / reload weights
    dico = Dictionary(reloaded['dico_id2word'], reloaded['dico_word2id'], reloaded['dico_counts'])
    encoder = TransformerModel(model_params, dico, is_encoder=True, with_output=True).cuda().eval()
    src_decoder = TransformerModel(model_params, dico, is_encoder=False, with_output=True).cuda().eval()
    tgt_decoder = TransformerModel(model_params, dico, is_encoder=False, with_output=True).cuda().eval()
    
    encoder.load_state_dict(reloaded['encoder'])
    src_decoder.load_state_dict(reloaded['src_decoder'])
    tgt_decoder.load_state_dict(reloaded['tgt_decoder'])   # use this to translate x86 to x86ir.
    ...
    # the translation process. Only translate the first 60 files
    for filename in os.listdir(params.src_folder):
    #for filename in os.listdir(params.src_folder):    
        #if filename.split('-')[0] in {'zlib',"diffffutils"}:
        ...
        if os.path.isfile(src_path):
            if os.path.exists(output_path):
                logger.info(f"Skipping file: {output_path} (output file already exists)")
                continue    
            ...
            with open (src_path,"r") as file: 
                for line_number, line in enumerate(file, start=1):
                    ...
                    if len(tokens)>params.max_len:
                        logger.info(f"Ignored line {line_number} in file {filename} since its length is {len(tokens)}")
                        continue
            ...
            #     src_sent.append(line)
            logger.info("Read %i sentences from the source file. Translating ..." % len(src_sent))
            logger.info(f"Translating file {filename} ...")
            
            with io.open(output_path, 'w', encoding='utf-8') as f:
            #f = io.open(params.output_folder, 'w', encoding='utf-8')
                ...
                for i in range(0, len(src_sent), params.batch_size):
                    ...
                    #    logger.info(f"word_id:{item}, length: {item.size()}")
                    # logger.info(f"words_ids length:{len(word_ids)}")
                    # logger.info(f"words_ids: {word_ids}")
                    lengths = torch.LongTensor([len(s) + 2 for s in word_ids])
                    # logger.info(f"the lengths is: {lengths}")
                    #the length of the batch is the longest sentence in the batch
                    #logger.info(f"lengths:{lengths}")
                    # logger.info(f"length.size() is:{lengths.size(0)}")
                    batch = torch.LongTensor(lengths.max().item(), lengths.size(0)).fill_(params.pad_index)
                    #batch.size(): [20,3] means there are 3 sentences, maximum length is 20.
                    #logger.info(f"batch:{batch}")
                    # for item in batch:
                    #     logger.info(f"sentence:{item}")
                    ...
                    #logger.info(f"encoded.size():{encoded.size()}")
                    encoded = encoded.transpose(0, 1)
                    #logger.info(f"encoded.size: {encoded.size()}")
                    decoded, dec_lengths = tgt_decoder.generate(encoded, lengths.cuda(), params.src_id, max_len=int(1.3 * lengths.max().item() + 10))
                    #logger.info(f"decoded.size:{decoded.size()},dec_lengths:{dec_lengths}")
...
if __name__ == '__main__':
    ...
    #params.model_path="dumped/x86-x86ir/20240515_32/periodic-3000.pth"
    params.model_path="dumped/s390x-x86/20240808_128/periodic-9.pth"

    params.dumped_path="dumped/test_translate/"
    #params.output_path="./dumped/test_translfate/x86-x86ir/translated_x86.txt"
    params.exp_name="s390x-x86"
    params.src_lang="s390x"
    params.tgt_lang="x86"
    
    #params.src_path="translate.x86ir"

    #params.src_folder="data/parallel_dataset_evaluation/x86-x86ir/x86_original/x86_zlib_O2/"
    #params.output_folder="data/parallel_dataset_evaluation/x86-x86ir/x86_128/"

    #params.src_folder="bpe_process/x86/x86_pb_bpe_cutted/"
    #params.output_folder="Malware_detection_files/param_study_x86/x86-x86ir-32/x86_benign"

    params.src_folder=f"original_dataset/gdl_pb_formatted/s390x_cutted_bpe"
    params.output_folder="Malware_detection_files/tensors/s390x_translation/20240808_128/s390x_benign"
    #params.output_folder=f"Malware_detection_files/tensors/x86_translation/x86_malware_20240613"

    #params.src_folder=f"original_dataset/ir_formatted/x86_cutted"
    #params.output_folder=f"Malware_detection_files/tensors/x86_translation/x86_20240725_64/x86_benign"
